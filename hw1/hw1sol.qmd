...
**Answer:** I display the URL of my GitHub repository [here](https://github.com/Zhangjiyin2000/biostat-203b-2024-winter).
...
I completed the CITI training. Here is the link to my [completion report](https://www.citiprogram.org/verify/?kc751f3e2-c959-4ab0-8744-89430dd87535-60370486) and here is the link to my [completion certificate](https://www.citiprogram.org/verify/?wb2be059f-01ad-4946-ad16-d3b2c3d94780-60370486).
...
ls -l ~/mimic/icu/
```
**Reason:** Gzip compression reduces the size of the files, making them smaller and more efficient for storage and transmission. The gzip compression is lossless, meaning that the decompressed data is identical to the original data.
...
Below the bash script is the output. It matches files starting with 'a', 'l', or 'pa' and ending with '.gz'.
...
Here is the bash script and the output of the bash script.
...
**Answer:** Here is the bash script and the output of the bash script.
...
```{bash} 
#| eval: true 
echo "First few lines of patients.csv.gz:"
zcat < ~/mimic/hosp/patients.csv.gz | head
```

```{bash}
#| eval: true
echo "Number of patients in patients.csv.gz:"
zcat < ~/mimic/hosp/patients.csv.gz | tail -n +2 | awk -F',' '{print $1}' | sort | uniq | wc -l
```

Therefore, the number of unique patients (identified by `subject_id`) in this data file **does not** match the number of patients listed in the `patients.csv.gz` file.
...
Compare compressed gz file size to the uncompressed file size. 

```{bash}
#| eval: true
echo "Size of compressed gz file:"
du -h ~/mimic/hosp/labevents.csv.gz
```
```{bash}
#| eval: true
echo "Size of uncompressed gz file:"
du -h labevents.csv
```
...
```{bash}
#| eval: true
echo "Run time for zcat:"
time zcat < ~/mimic/hosp/labevents.csv.gz | wc -l
```

```{bash}
#| eval: true
echo "Run time for wc:"
time wc -l labevents.csv
```
Delete the large labevents.csv file after the exercise.
...
Compressed files (labevents.csv.gz) generally occupy less disk space compared to their uncompressed counterparts (labevents.csv).
Compression is beneficial when storage space is a critical consideration, especially for large datasets.

**Speed:**

Reading compressed files requires decompression, introducing additional computational overhead.
Reading uncompressed files is faster as there is no need for decompression.

**Trade-Off:**

Compression trades storage space for slower access times. If storage space is limited, compression is beneficial. However, if fast access and reading times are critical, using uncompressed files may be preferred.

**Considerations:**

The choice between compression and uncompressed files depends on the specific use case, available storage, and the frequency of data access.
For datasets that are read frequently and where storage is not a major concern, uncompressed files might be preferable for faster access times.
...
cat test2.txt
```
**Answer:** The first command overwrites the file `test1.txt` if the file exists. The second command appends the text to the file `test2.txt` if the file exists. To clarify, the use of > signifies file overwriting, while >> indicates appending content to the existing file.
