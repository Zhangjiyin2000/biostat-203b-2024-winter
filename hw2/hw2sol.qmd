...
The code did not finish running after 5 minutes. Therefore, I aborted the 
program. The reason is also that my memory size is 8GB and I expect this 
labevents.csv.gz file is too large to fit into my memory.
...
```{r}
# Read the Parquet file, select columns and filter rows
...
Ingest the Parquet file, convert it to a DuckDB table by [`arrow::to_duckdb`](https://arrow.apache.org/docs/r/reference/to_duckdb.html), 
...
```{r}
# Display the number of rows and the first 10 rows
cat("Number of Rows (Parquet):", nrow(labevents_duckdb), "\n")
